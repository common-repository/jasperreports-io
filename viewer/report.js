...
var $overlayUrl = $reportContainer.data('overlayUrl');

function jrio_open_viewer()
{
    window.parent.document.getElementById('jrio_viewer_frame').src = $overlayUrl;
    window.parent.document.getElementById('jrio_viewer_frame').style.width = '100%'; // twentytwenty theme needs this
    window.parent.document.getElementById('jrio_viewer').style.display = 'block';
}
...
jrio(function(jrioClient) {
    ...
    var observerM = new MutationObserver(function(mutationsList, observer) {
        for(let mutation of mutationsList) {
            if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
                for(let node of mutation.addedNodes) {
                    if (node.tagName === "TABLE" && node.className === "jrPage") {
                        resizeFrame();
                        break;
                    }
                }
            ...
            if (mutation.type === "attributes" &&
                    ...
                    mutation.target.className === "jrPage") {
                
                resizeFrame();
            }
        }
    });
    observerM.observe(document.getElementById('reportContainer'), { attributes: true, childList: true, subtree: true });

    var options = {
        root: document.documentElement
    }
    
    var observerI = new IntersectionObserver(handleIntersect, options);
    observerI.observe(document.getElementById('reportContainer'));
    
    function handleIntersect(entries, observerI) {
    ...
    function resizeFrame() {
        ...
        var frame = window.parent.document.getElementById(urlParams[frameIdParam]);
        frame.style.height = '0px'; // force this to trigger scrollWidth
        frame.style.height = document.body.scrollHeight + 'px';
    }

    if (urlParams[scaleParam] && urlParams[scaleParam] !== 'none') {
        reportConfig.scale = urlParams[scaleParam];
    }

    var interactive = false;
    if (urlParams[interactiveParam] && urlParams[interactiveParam] == 'true') {
        interactive = true;
    }
    reportConfig.defaultJiveUi = { enabled: interactive, floatingTableHeadersEnabled: interactive, floatingCrosstabHeadersEnabled: interactive };
    ...
    function clickHandler(ev, hyperlinkData) {
        if ("ReportExecution" === hyperlinkData.type) {
            ...
            if (hParams && hParams[hReportParam]) {
                var href = "report?" + reportUriParam + "=" + hParams[hReportParam],
    ...
    function buildViewerUrl(params) {
        var href = "report?" + reportUriParam + "=" + urlParams[reportUriParam];
